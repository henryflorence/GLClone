...
GLboolean gltraceInit()
{
   ...
   freeaddrinfo(peer);
   	...
   	netBuffer = bigBuffer = (NLbyte *)malloc(100 * PACKET_LENGTH);
	readyToSend = (int *)malloc(100 * sizeof(int));
	packetCount = 0;

	//netBuffer[0] = (unsigned char)ID_USER_PACKET_ENUM;
	count = 0;
	writeShort(netBuffer,count,400);
	writeShort(netBuffer,count,0);
	//OpenRakNetSocket();
...
int SendPacket() {
   ...
   //{
      while (ssize < size)
      {
         if (UDT::ERROR == (ss = UDT::send(client, netBuffer + ssize, size - ssize, 0)))
         {
...
int SendStream() {
	writeShort(netBuffer,count,500);
	
	SendPacket();
	/*int overlap = count - PACKET_LENGTH;

	if(++packetCount > 99) {
		netBuffer = bigBuffer;
		packetCount = 0;
		if(count > PACKET_LENGTH)
			memcpy(bigBuffer,netBuffer,overlap);
	} else {
		netBuffer += PACKET_LENGTH;
	}*/
	count = 0;
	writeShort(netBuffer,count,400);
	//writeShort(netBuffer,count,overlap);
	//count = overlap + 2;
	return 1;
}
