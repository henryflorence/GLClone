...
UPNPNATInternal::UPNPNATInternal(RakNet::RakString whichInterface,RakNet::TimeMS timeout)
{
...
void UPNPNATInternal::Restart(RakNet::RakString whichInterface,RakNet::TimeMS timeout)
{
	tcpInterface.Stop();
...
bool UPNPNATInternal::TcpConnect(const char * host,unsigned short int port)
{
	...
	RakNet::TimeMS stopWaiting = RakNet::GetTimeMS() + timeOut;
...
bool UPNPNATInternal::Discovery(void)
{
	...
	RakNet::TimeUS timeOut;
	RakNet::TimeMS stopWaiting = RakNet::GetTimeMS() + 5000;//Reply times can vary, this value seems to be a good buffer to catch
...
bool UPNPNATInternal::GetDescription()
{
	...
	//send request
	tcpInterface.Send(httpRequest.C_String(),(int) httpRequest.GetLength(),RakNet::UNASSIGNED_SYSTEM_ADDRESS,true);
...
RakNet::RakString UPNPNATInternal::WaitAndReturnResponse(int waitTime)
{
	...
	RakNet::TimeMS stopWaiting = RakNet::GetTimeMS() + waitTime;
	while (RakNet::GetTimeMS() < stopWaiting)
	{
...
bool UPNPNATInternal::ParseDescription()
{
	...
	if(controlUrl.Find("http://")==-1&&controlUrl.Find("HTTP://")==-1)
		controlUrl=baseUrl+controlUrl;
	if(serviceDescribeUrl.Find("http://")==-1&&serviceDescribeUrl.Find("HTTP://")==-1)
