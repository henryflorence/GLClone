...
Lobby2Server::Lobby2Server()
{
	...
	roomsPlugin=0;
	roomsPluginAddress=RakNet::UNASSIGNED_SYSTEM_ADDRESS;
}
Lobby2Server::~Lobby2Server()
{
	Clear();
...
void Lobby2Server::Update(void)
{
	...
	if (threadPool.HasOutputFast() && threadPool.HasOutput())
	{
		...
		if (c.returnToSender)
		{
			...
			if (c.callerSystemAddress==RakNet::UNASSIGNED_SYSTEM_ADDRESS)
			{
				unsigned int i;
				if (c.callerUserId!=0)
				{
					for (i=0; i < users.Size(); i++)
					{
						if (users[i]->callerUserId==c.callerUserId)
						{
							...
							if (c.requiredConnectionAddress!=RakNet::UNASSIGNED_SYSTEM_ADDRESS)
							{
								...
								if (objectExists==false)
								{
									if (c.deallocMsgWhenDone)
										RakNet::OP_DELETE(c.lobby2Message, _FILE_AND_LINE_);
									return;
								}
							}
							break;
				...
				if (c.callerSystemAddress==RakNet::UNASSIGNED_SYSTEM_ADDRESS && c.callingUserName.IsEmpty()==false)
				{
					for (i=0; i < users.Size(); i++)
					{
...
void Lobby2Server::OnClosedConnection(SystemAddress systemAddress, RakNetGUID rakNetGUID, PI2_LostConnectionReason lostConnectionReason )
{
	...
	if (index!=-1)
	{
		...
		command.callerSystemAddress=RakNet::UNASSIGNED_SYSTEM_ADDRESS;
...
void Lobby2Server::Clear(void)
{
	...
	threadActionQueueMutex.Lock();
	threadActionQueue.Clear(_FILE_AND_LINE_);
	threadActionQueueMutex.Unlock();
}
void Lobby2Server::AddAdminAddress(SystemAddress addr)
{
	adminAddresses.Insert(addr,addr,false, _FILE_AND_LINE_ );
}
bool Lobby2Server::IsAdminAddress(SystemAddress addr)
{
	return addr==RakNet::UNASSIGNED_SYSTEM_ADDRESS || adminAddresses.HasData(addr);
}
...
{
	adminAddresses.Clear(false, _FILE_AND_LINE_);
}
void Lobby2Server::AddRankingAddress(SystemAddress addr)
{
	rankingAddresses.Insert(addr,addr,false, _FILE_AND_LINE_ );
}
bool Lobby2Server::IsRankingAddress(SystemAddress addr)
{
	return addr==RakNet::UNASSIGNED_SYSTEM_ADDRESS || rankingAddresses.HasData(addr);
}
...
{
	rankingAddresses.Clear(false, _FILE_AND_LINE_);
}
...
void Lobby2Server::LogoffFromRooms(User *user)
{
	...
	if (roomsPlugin)
	{
		roomsPlugin->LogoffRoomsParticipant(user->userName, RakNet::UNASSIGNED_SYSTEM_ADDRESS);
	}
	else if (roomsPluginAddress!=RakNet::UNASSIGNED_SYSTEM_ADDRESS)
	{
		RakNet::BitStream bs;
...
void Lobby2Server::OnLogin(Lobby2ServerCommand *command, bool calledFromThread)
{
	if (calledFromThread)
	{
		...
		ta.command=*command;
		threadActionQueueMutex.Lock();
		threadActionQueue.Push(ta, _FILE_AND_LINE_ );
		threadActionQueueMutex.Unlock();
		return;
	}
	...
	users.InsertAtIndex(user, insertionIndex, _FILE_AND_LINE_ );

#if defined(__INTEGRATE_LOBBY2_WITH_ROOMS_PLUGIN)
	// Tell the rooms plugin about the login event
	if (roomsPlugin)
	{
		roomsPlugin->LoginRoomsParticipant(user->userName, user->systemAddress, user->guid, RakNet::UNASSIGNED_SYSTEM_ADDRESS);
	}
	else if (roomsPluginAddress!=RakNet::UNASSIGNED_SYSTEM_ADDRESS)
	{
		RakNet::BitStream bs;
...
{
	// Could be RakNet::UNASSIGNED_SYSTEM_ADDRESS if locally generated due to disconnect
...
void Lobby2Server::RemoveUser(unsigned int index)
{
	...
	while (i < threadPool.InputSize())
	{
		...
		{
			if (command.deallocMsgWhenDone)
				RakNet::OP_DELETE(command.lobby2Message, _FILE_AND_LINE_);
