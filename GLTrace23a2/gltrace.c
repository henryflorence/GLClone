...
void OpenServerSocket()
{
	...
	char      client[16] = "127.0.0.1:25000";

	print("nlGetString(NL_VERSION) = %s\n", nlGetString(NL_VERSION));
    print("nlGetString(NL_NETWORK_TYPES) = %s\n\n", nlGetString(NL_NETWORK_TYPES));

	if(!nlSelectNetwork(NL_IP))
		print("Could not select network\n");
	
	serversock = nlOpen(0, NL_RELIABLE); /* just a random port number ;) */

    if(serversock == NL_INVALID)
        print("Invalid server socket\n");

	nlGetLocalAddr(serversock, &addr);
    print("our address is %s\n", nlAddrToString(&addr, string));
	nlStringToAddr((NLchar*)client, &addr);
    printf("client address is %s\n", nlAddrToString(&addr, string));

    if(!nlConnect(serversock, &addr))       /* let's listen on this socket */
    {
        nlClose(serversock);
        print("Could not connect socket to client\n");
    }
    nlGetLocalAddr(serversock, &addr);
...
void print_error(NLenum errCode) {
	switch(errCode) {
			case NL_NULL_POINTER: print("NL_NULL_POINTER"); break;
			case NL_INVALID_SOCKET: print("NL_INVALID_SOCKET"); break;
			case NL_BUFFER_SIZE: print("NL_BUFFER_SIZE"); break;
			case NL_CON_REFUSED: print("NL_CON_REFUSED"); break;
			case NL_CON_PENDING: print("NL_CON_PENDING"); break;
			case NL_SYSTEM_ERROR: print("NL_SYSTEM_ERROR"); break;
			case NL_MESSAGE_END: print("NL_MESSAGE_END"); break;
		}
}
int SendPacket(int sendCount) {
	int rtn;
	NLenum result;
	while((rtn = nlWrite(serversock, netBuffer, sendCount)) == NL_INVALID ) {
		result = nlGetError();
		print("sendingStream failed: %i - ", result);
		print_error(result);
		print("\n");
		Sleep(1000);
	}
...
int SendStream() {
	int rtn, sendCount = count; //, sentCount;
	NLenum result;

	//end of packet
	AddIntToStream(500);
	AddIntToStream(packetCount++);
	(rtn = SendPacket(sendCount)); // < sendCount) {
			//&& (sentCount += rtn) < count) { 
		//sendCount = sendCount - rtn;
		print("missing bytes: %i: %i < %i\n",count,rtn,sendCount);
		//Sleep(1000);
	//}
...
void AddIntToStream(int value) {
	writeShort(netBuffer, count, value);
	//print("-------------adding to stream-------------------");
	//print(string);
}
void AddDoubleToStream(NLdouble value) {
	writeDouble(netBuffer, count, value);
}
void AddByteToStream(NLbyte value) {
	writeByte(netBuffer, count, value);
}
void StreamBytes(const GLvoid *ptr, int n) {
	memcpy((NLubyte *)netBuffer[count], (NLubyte *)ptr, n);
	count += n;
	//writeBlock(netBuffer, count, ptr, n);
}
void AddShortToStream(NLshort value) {
	writeShort(netBuffer, count, value);
}
void AddFloatToStream(NLfloat value) {
	writeNoSwapFloat(netBuffer, count, value);
}
void AddLongToStream(NLulong value) {
	writeLong(netBuffer, count, value);
}
void AddToStream(char *string) {
	writeString(netBuffer, count, string);
	count--; //remove trailing null
	//print("-------------adding to stream-------------------");
	//print(string);
}
#if defined WIN32 || defined WIN64
